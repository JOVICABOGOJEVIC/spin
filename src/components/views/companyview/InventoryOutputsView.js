...
const InventoryOutputsView = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const [itemsRes, transactionsRes, jobsRes] = await Promise.all([
        getInventoryItems(),
        getWarehouseTransactions({ type: 'OUTPUT', ...dateFilter }),
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      await createOutputTransaction(formData);
      toast.success('Izlaz robe je registrovan');
      setShowForm(false);
      setFormData({
        itemId: '',
        quantity: 0,
      ...
      toast.error(error.response?.data?.message || 'Greška pri čuvanju izlaza');
    }
  };
        ...
        <div className="bg-gray-800 rounded-xl shadow-xl p-6 border border-gray-700">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
              <h1 className="text-2xl font-bold text-white flex items-center gap-2">
                ...
                {t('inventory.outputs.title', 'Izlazi robe')}
              </h1>
              <p className="text-sm text-gray-400 mt-1">
                {t('inventory.outputs.subtitle', 'Registracija izlaza robe iz magacina')}
              </p>
            </div>
            <button
            ...
            >
              ...
              {t('inventory.outputs.addOutput', 'Dodaj izlaz')}
            </button>
          </div>
          ...
          <div className="mt-6 space-y-4">
            <div className="relative">
              ...
              <input
                type="text"
                placeholder={t('inventory.outputs.search', 'Pretraži izlaze...')}
                value={searchTerm}
          ...
          <div className="bg-gray-800 rounded-xl shadow-xl p-6 border border-gray-700">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-xl font-bold text-white">
                {t('inventory.outputs.newOutput', 'Novi izlaz robe')}
              </h2>
              <button
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  {selectedItem && (
                    <span className="text-xs text-gray-400 mt-1">
                      {t('inventory.available', 'Dostupno')}: {availableQuantity} {selectedItem.unit}
                    </span>
                  )}
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-300 mb-1">
                    {t('inventory.outputs.documentNumber', 'Broj dokumenta')} *
                  </label>
                  <input
                    type="text"
                    name="documentNumber"
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-300 mb-1">
                    {t('inventory.outputs.documentDate', 'Datum dokumenta')} *
                  </label>
                  <input
                    type="date"
                    name="documentDate"
                ...
                <div>
                  ...
                  <select
                    ...
                    className="w-full px-3 py-2 bg-gray-700 border border-gray-600 rounded text-white focus:outline-none focus:ring-2 focus:ring-blue-500"
                  >
                    <option value="">{t('common.select', 'Izaberi posao (opciono)...')}</option>
                    {jobs.map(job => (
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-300 mb-1">
                    {t('inventory.outputs.reason', 'Razlog')}
                  </label>
                  <input
                    type="text"
                    name="reason"
            ...
            <table className="w-full">
              <thead className="bg-gray-700">
                <tr>
                  ...
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-300 uppercase tracking-wider">
                    {t('inventory.outputs.documentDate', 'Datum')}
                  </th>
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-300 uppercase tracking-wider">
                    {t('inventory.name', 'Artikal')}
                  </th>
                  ...
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-300 uppercase tracking-wider">
                    {t('inventory.outputs.customer', 'Kupac')}
                  </th>
                </tr>
              </thead>
                ...
                {filteredTransactions.length === 0 ? (
                  <tr>
                    <td colSpan="6" className="px-4 py-8 text-center text-gray-400">
                      {t('inventory.outputs.noOutputs', 'Nema izlaza robe')}
                    </td>
                  </tr>
                ) : (
...
export default InventoryOutputsView;
